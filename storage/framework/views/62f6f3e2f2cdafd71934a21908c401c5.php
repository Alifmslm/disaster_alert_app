...
<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-5 px-6">
    <form method="GET" action="<?php echo e(route('officer.kelola-data.penanggulangan.index')); ?>" class="flex flex-col md:flex-row md:items-center gap-3 w-full lg:w-auto">
    ...
    <div class="flex items-center justify-between lg:justify-end gap-4 w-full lg:w-auto">
        <a href="<?php echo e(route('officer.kelola-data.penanggulangan.create')); ?>" class="bg-[#FF7F3E] hover:bg-[#e66a2e] text-white py-2 px-4 rounded-lg text-[11px] font-black uppercase tracking-wide transition-all shadow-sm flex items-center gap-2">
...
<div class="overflow-x-auto w-full">
        ...
        <tbody>
            ...
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="py-[18px] px-5 border-t border-slate-100 text-slate-900 text-[13px] font-black align-middle max-w-[260px]">
                    <?php echo e($note->title); ?><br>
                    <span class="text-[10px] text-slate-400 font-medium">Area: <?php echo e($note->affected_area); ?></span>
                </td>
                <td class="py-[18px] px-5 border-t border-slate-100 text-slate-600 text-[13px] align-middle">
                    <?php echo e(strtoupper(str_replace('_', ' ', $note->disaster_type))); ?>

                </td>
                <td class="py-[18px] px-5 border-t border-slate-100 text-slate-600 text-[13px] align-middle">
                    <?php if($note->disasterReport): ?>
                        <div class="flex flex-col">
                            <span class="text-blue-600 font-black text-[11px] uppercase tracking-tighter flex items-center gap-1">
                                <i class="fas fa-file-alt"></i>
                                Laporan: <?php echo e($note->disasterReport->location_name ?? 'Titik Peta'); ?>

                            </span>
                            <span class="text-[10px] text-slate-400"><?php echo e($note->disasterReport->occurred_at ? $note->disasterReport->occurred_at->format('d/m/Y') : '-'); ?></span>
                        </div>
                    <?php elseif($note->disasterEvent): ?>
                        <div class="flex flex-col">
                            <span class="text-orange-600 font-black text-[11px] uppercase tracking-tighter flex items-center gap-1">
                                <i class="fas fa-link"></i>
                                Kejadian: <?php echo e($note->disasterEvent->title); ?>

                            </span>
                            <span class="text-[10px] text-slate-400"><?php echo e($note->disasterEvent->occurred_at ? $note->disasterEvent->occurred_at->format('d/m/Y') : '-'); ?></span>
                        </div>
                    <?php else: ?>
                ...
                <td class="py-[18px] px-5 border-t border-slate-100 text-[13px] align-middle">
                    <div class="flex items-center gap-2">
                        <a href="<?php echo e(route('officer.kelola-data.penanggulangan.edit', $note)); ?>" 
                        ...
                        <a href="<?php echo e(route('officer.kelola-data.penanggulangan.show', $note)); ?>" 
                        ...
                        <form action="<?php echo e(route('officer.kelola-data.penanggulangan.destroy', $note)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
    ...
    <div class="p-5"><?php echo e($notes->links()); ?></div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('pages.officer.kelola-data.manage-data', ['activeTab' => 'penanggulangan'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH Z:\internet_download\Kuliah Alif\SEMESTER 4\MANPROSI\disaster_alert_app\resources\views\pages\officer\kelola-data\catatan-penanggulangan.blade.php ENDPATH**/ ?>