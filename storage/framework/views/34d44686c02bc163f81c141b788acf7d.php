...
<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-5 px-6">
    <form id="filterForm" method="GET" action="<?php echo e(route('officer.kelola-data.faskes.index')); ?>" class="flex flex-col md:flex-row md:items-center gap-3 w-full lg:w-auto">
        ...
        <select name="type" onchange="document.getElementById('filterForm').submit()" class="w-full md:w-auto min-w-[160px] border border-slate-200 rounded-lg bg-white text-slate-600 py-2.5 px-3 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer">
            ...
            <?php $__currentLoopData = $types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($type->value ?? $type); ?>" <?php echo e(request('type') == ($type->value ?? $type) ? 'selected' : ''); ?>>
                    <?php echo e(is_object($type) && method_exists($type, 'label') ? $type->label() : $type); ?>
        ...
        <select name="status" onchange="document.getElementById('filterForm').submit()" class="w-full md:w-auto min-w-[160px] border border-slate-200 rounded-lg bg-white text-slate-600 py-2.5 px-3 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer">
            ...
            <option value="active" <?php echo e(request('status') == 'active' ? 'selected' : ''); ?>>Operasional</option>
        ...
        <?php if(request()->filled('type') || request()->filled('status')): ?>
            <a href="<?php echo e(route('officer.kelola-data.faskes.index')); ?>" class="text-[10px] font-black text-red-500 hover:text-red-700 uppercase tracking-widest transition-colors">Reset</a>
        <?php endif; ?>
    </form>
    ...
    <div class="flex items-center justify-between lg:justify-end gap-4 w-full lg:w-auto">
        ...
        <a href="<?php echo e(route('officer.kelola-data.faskes.create')); ?>" class="bg-[#FF7F3E] hover:bg-[#e66a2e] text-white py-2.5 px-4 rounded-lg text-[11px] font-black uppercase tracking-wide transition-all shadow-sm flex items-center gap-2">
...
<div class="overflow-x-auto w-full">
    <table class="w-full min-w-[980px] border-collapse">
        <thead class="bg-slate-100">
            <tr>
                <th class="py-[17px] px-6 text-slate-700 text-[11px] font-black tracking-[0.08em] text-left uppercase whitespace-nowrap">Nama Fasilitas</th>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $places; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $place): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="py-[18px] px-5 border-t border-slate-100 text-[13px] align-middle">
                        <span class="inline-flex items-center justify-center rounded-full py-1 px-2.5 text-[10px] font-black uppercase <?php echo e(is_object($place->type) && method_exists($place->type, 'color') ? $place->type->color() : 'bg-slate-100 text-slate-600'); ?>">
                            <?php echo e(is_object($place->type) && method_exists($place->type, 'label') ? $place->type->label() : $place->type); ?>
                    ...
                    <td class="py-[18px] px-5 border-t border-slate-100 text-[13px] align-middle">
                        <div class="flex items-center gap-2">
                            <a href="<?php echo e(route('officer.kelola-data.faskes.edit', $place->id)); ?>" class="border border-slate-200 rounded bg-white text-slate-700 py-1.5 px-3 text-[10px] font-black cursor-pointer hover:border-orange-400 transition-colors">
                                EDIT
                            </a>
                            
                            <form action="<?php echo e(route('officer.kelola-data.faskes.destroy', $place->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data fasilitas ini secara permanen?');">
                                <?php echo csrf_field(); ?>
                ...
                <tr>
                    <td colspan="5" class="py-12 text-center">
                        <div class="flex flex-col items-center justify-center text-slate-400">
                            ...
                            <p class="text-sm font-bold">Belum ada data fasilitas kesehatan.</p>
                        </div>
                    </td>
                </tr>
            <?php endif; ?>
...
<?php echo $__env->make('pages.officer.kelola-data.manage-data', ['activeTab' => 'faskes'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH Z:\internet_download\Kuliah Alif\SEMESTER 4\MANPROSI\disaster_alert_app\resources\views\pages\officer\kelola-data\fasilitas-kesehatan.blade.php ENDPATH**/ ?>