...
<div class="min-h-screen bg-slate-50 p-6 md:p-8 font-sans text-slate-800">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        {{-- Form diarahkan ke route update dengan parameter $penanggulangan --}}
        <form action="{{ route('officer.kelola-data.penanggulangan.update', $penanggulangan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white" value="{{ old('title', $penanggulangan->title) }}">
                ...
                <div>
                    ...
                    <select name="disaster_type" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white">
                        @foreach(['banjir', 'tanah_longsor', 'kebakaran', 'gempa', 'angin_kencang'] as $type)
                            <option value="{{ $type }}" {{ old('disaster_type', $penanggulangan->disaster_type) == $type ? 'selected' : '' }}>
                                {{ ucwords(str_replace('_', ' ', $type)) }}
                            </option>
                        @endforeach
                    </select>
                    @error('disaster_type') <p class="mt-1 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">Area Terdampak</label>
                    <input type="text" name="affected_area" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white" value="{{ old('affected_area', $penanggulangan->affected_area) }}">
                    @error('affected_area') <p class="mt-1 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-2">
                    <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">Tanggal Tindakan</label>
                    <input type="date" name="action_date" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white" value="{{ old('action_date', $penanggulangan->action_date ? \Carbon\Carbon::parse($penanggulangan->action_date)->format('Y-m-d') : '') }}">
                    @error('action_date') <p class="mt-1 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-2">
                    <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">Deskripsi Penanggulangan</label>
                    <textarea name="description" rows="5" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none resize-none focus:border-orange-400 focus:bg-white">{{ old('description', $penanggulangan->description) }}</textarea>
