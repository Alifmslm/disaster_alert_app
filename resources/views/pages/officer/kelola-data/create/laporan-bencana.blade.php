...
<div class="min-h-screen bg-slate-50 p-6 md:p-8 font-sans text-slate-800">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('officer.kelola-data.laporan.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white @error('type') border-red-400 @enderror">
                        ...
                        <option value="banjir" {{ old('type') == 'banjir' ? 'selected' : '' }}>Banjir</option>
                        <option value="tanah_longsor" {{ old('type') == 'tanah_longsor' ? 'selected' : '' }}>Tanah Longsor</option>
                        <option value="kebakaran" {{ old('type') == 'kebakaran' ? 'selected' : '' }}>Kebakaran</option>
                        <option value="gempa" {{ old('type') == 'gempa' ? 'selected' : '' }}>Gempa</option>
                        <option value="angin_kencang" {{ old('type') == 'angin_kencang' ? 'selected' : '' }}>Angin Kencang</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white @error('status') border-red-400 @enderror">
                        <option value="submitted" {{ old('status', 'submitted') == 'submitted' ? 'selected' : '' }}>Darurat (Baru)</option>
                        <option value="verified" {{ old('status') == 'verified' ? 'selected' : '' }}>Divalidasi</option>
                        <option value="in_progress" {{ old('status') == 'in_progress' ? 'selected' : '' }}>Diproses</option>
                        <option value="handled" {{ old('status') == 'handled' ? 'selected' : '' }}>Selesai</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="reporter_name" value="{{ old('reporter_name') }}" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white @error('reporter_name') border-red-400 @enderror">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="datetime-local" name="occurred_at" value="{{ old('occurred_at') }}" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white @error('occurred_at') border-red-400 @enderror">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" required class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none resize-none focus:border-orange-400 focus:bg-white @error('description') border-red-400 @enderror">{{ old('description') }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="location_name" id="location_name" value="{{ old('location_name') }}" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold outline-none focus:border-orange-400 focus:bg-white @error('location_name') border-red-400 @enderror">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <input type="text" id="latitude" name="latitude" value="{{ old('latitude') }}" readonly class="w-full rounded-lg border border-slate-200 bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-500 outline-none">
                            @error('latitude') <p class="mt-1 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <input type="text" id="longitude" name="longitude" value="{{ old('longitude') }}" readonly class="w-full rounded-lg border border-slate-200 bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-500 outline-none">
...
document.addEventListener('DOMContentLoaded', function () {
    let defaultLat = {{ old('latitude', -6.2088) }};
    let defaultLng = {{ old('longitude', 106.8456) }};
