...
<div class="min-h-screen bg-slate-50 p-6 md:p-8 font-sans text-slate-800">
    ...
    <form action="{{ route('officer.kelola-data.evakuasi.store') }}" method="POST">
        @csrf
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select id="disaster_type" name="disaster_type"
                                    ...
                                    <option value="" disabled {{ old('disaster_type') ? '' : 'selected' }}>Pilih tipe bencana</option>
                                    <option value="banjir"        {{ old('disaster_type') === 'banjir'        ? 'selected' : '' }}>Banjir</option>
                                    <option value="tanah_longsor" {{ old('disaster_type') === 'tanah_longsor' ? 'selected' : '' }}>Tanah Longsor</option>
                                    <option value="kebakaran"     {{ old('disaster_type') === 'kebakaran'     ? 'selected' : '' }}>Kebakaran</option>
                                    <option value="gempa"         {{ old('disaster_type') === 'gempa'         ? 'selected' : '' }}>Gempa Bumi</option>
                                    <option value="angin_kencang" {{ old('disaster_type') === 'angin_kencang' ? 'selected' : '' }}>Angin Kencang</option>
                                    <option value="lainnya"       {{ old('disaster_type') === 'lainnya'       ? 'selected' : '' }}>Lainnya</option>
                                </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                class="w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700 placeholder:text-slate-400 outline-none transition focus:border-orange-400 focus:bg-white focus:ring-2 focus:ring-orange-500/10 @error('description') border-red-400 bg-red-50 @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs font-bold text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="flex flex-col gap-3">
                                <div>
                                    ...
                                    <input type="number" id="start_latitude" name="start_latitude"
                                        value="{{ old('start_latitude') }}" step="0.0000001" placeholder="-6.2088"
                                        class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 placeholder:text-slate-300 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-500/10 @error('start_latitude') border-red-400 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="number" id="start_longitude" name="start_longitude"
                                        value="{{ old('start_longitude') }}" step="0.0000001" placeholder="106.8456"
                                        class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 placeholder:text-slate-300 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-500/10 @error('start_longitude') border-red-400 @enderror">
                            ...
                            <div class="flex flex-col gap-3">
                                <div>
                                    ...
                                    <input type="number" id="end_latitude" name="end_latitude"
                                        value="{{ old('end_latitude') }}" step="0.0000001" placeholder="-6.1751"
                                        class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 placeholder:text-slate-300 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-500/10 @error('end_latitude') border-red-400 @enderror">
                                    @error('end_latitude')
                                        <p class="mt-1 text-xs font-bold text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    ...
                                    <input type="number" id="end_longitude" name="end_longitude"
                                        value="{{ old('end_longitude') }}" step="0.0000001" placeholder="106.8272"
                                        class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 placeholder:text-slate-300 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-500/10 @error('end_longitude') border-red-400 @enderror">
                                    @error('end_longitude')
                                        <p class="mt-1 text-xs font-bold text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
...
<script>
    ...
    @if(old('start_latitude') && old('start_longitude'))
        setStartPoint({{ old('start_latitude') }}, {{ old('start_longitude') }});
    @endif
    @if(old('end_latitude') && old('end_longitude'))
        setEndPoint({{ old('end_latitude') }}, {{ old('end_longitude') }});
    @endif
